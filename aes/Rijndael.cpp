...
static const int nrounds = sizeof(rcon) / sizeof(rcon[0]);

static inline DWORD GETDWORD(const BYTE *plaintext)
{
	return (((DWORD)(plaintext)[0] << 24) ^
		((DWORD)(plaintext)[1] << 16) ^
		((DWORD)(plaintext)[2] << 8) ^
		((DWORD)(plaintext)[3]));
}

static inline void PUTDWORD(BYTE *ciphertext, DWORD st) {
...
void Rijndael_set_key_decrypt(DWORD *rk, const BYTE *key)
{
	...
	Rijndael_set_key_encrypt(rk, key);
